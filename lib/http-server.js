    ...
    before: (options.before || []).concat([
      ...
      function (req, res) {
        ...
        if (options.pushState) {
          ignorePaths = ignorePaths.concat(Object.keys(mutableExtras).map(function (path) { return '/' + path; }));
          if (options.autoIndex !== 'false' && ignorePaths.indexOf(url.parse(req.url).pathname) === -1) {
            if (!fs.existsSync(options.root + url.parse(req.url).pathname) && !fs.existsSync(options.root + url.parse(req.url).pathname + indexPath)) {
              req.url = indexPath;
            }
          }
        }
